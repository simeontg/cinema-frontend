...
export const useGetTrendedMovies = <ReturnData = TrendedMovie[] | null>(): HookReturnData<ReturnData> => {
    const { data, isFetching, isError, isSuccess } = useQuery({
        queryKey: ['trending'],
        queryFn: async () => {
            const response = await $api.get<GetTrendedMovieDTO[]>(`${MOVIES_BASE_URL}/movies/trended`);
